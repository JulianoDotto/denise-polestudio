...
import { deleteClass, toggleClassStatus } from '@/lib/admin/actions'
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{total} aula(s)</p>
        </div>
        <Link
        ...
        {items.map((item) => (
          <div key={item.id} className="rounded-3xl border bg-white p-4">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              <DeleteButton action={deleteClass} id={item.id} />
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="flex gap-2">
          {Array.from({ length: totalPages }).map((_, index) => {
            ...
            return (
              <Link
                key={pageNumber}
                href={`/admin/aulas?${params.toString()}`}
