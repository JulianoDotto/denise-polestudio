...
export default async function EditWorkshopPage({
  ...
  if (!item || item.type !== 'WORKSHOP') return notFound()

  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold">Editar workshop</h1>
        <p className="text-sm text-muted-foreground">Atualize os dados do workshop.</p>
      </div>

      <form action={updateWorkshop} className="grid gap-6">
              ...
              content: (
                <section className="rounded-3xl border bg-white p-5">
                  ...
                  <div className="mt-4 grid gap-4">
                    ...
                    <ImageUrlField name="coverUrl" label="Imagem" defaultValue={item.coverUrl || ''} />
                    <label className="flex items-center gap-2 text-sm">
                      <input type="checkbox" name="isActive" defaultChecked={item.isActive} className="h-4 w-4" />
                      <span>Workshop ativo</span>
                    </label>
                  </div>
                </section>
              ),
