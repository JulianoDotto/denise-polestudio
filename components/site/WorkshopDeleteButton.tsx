...
import { deleteWorkshopInline } from '@/lib/admin/actions'

type WorkshopDeleteButtonProps = {
  id: string
  title: string
}
...
export default function WorkshopDeleteButton({ id, title }: WorkshopDeleteButtonProps) {
  const [state, formAction, pending] = useActionState(
    deleteWorkshopInline,
    initialState,
  )
    ...
    <Dialog>
      <DialogTrigger asChild>
        <button
          ...
          aria-label={TEXTS.WORKSHOPS_DELETE_ICON_ARIA_1}
        >
      ...
      <DialogContent className="max-w-sm rounded-3xl border bg-white p-6">
        <DialogHeader className="text-left">
          <DialogTitle className="text-zinc-900">
            {TEXTS.WORKSHOPS_DELETE_TITLE_1}
          </DialogTitle>
          <DialogDescription className="text-sm text-zinc-600">
            {TEXTS.WORKSHOPS_DELETE_DESCRIPTION_1.replace('{title}', title)}
          </DialogDescription>
        </DialogHeader>
        {state.error ? (
          <p className="mt-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm text-rose-700">
            {TEXTS.WORKSHOPS_DELETE_ERROR_1}
          </p>
        ) : null}
        <form action={formAction} className="mt-4 flex gap-3">
          ...
          >
            {pending
              ...
              : TEXTS.WORKSHOPS_DELETE_CONFIRM_1}
          </button>
          <DialogClose asChild>
            <button
              type="button"
            ...
            >
              {TEXTS.WORKSHOPS_DELETE_CANCEL_1}
            </button>
          </DialogClose>
        </form>
      </DialogContent>
